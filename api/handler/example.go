...
// Example.Call is called by the API as /api/example/call with post body {"name": "foo"}
func (e *Example) Call(ctx context.Context, req *api.Request, rsp *api.Response) error {
	...
	if !ok {
		return errors.InternalServerError("go.micro.api.api.example.call", "example client not found")
	}
	...
	if err != nil {
		return errors.InternalServerError("go.micro.api.api.example.call", err.Error())
	}
